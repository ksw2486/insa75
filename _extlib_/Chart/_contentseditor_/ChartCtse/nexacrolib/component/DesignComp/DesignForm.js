...
if (!nexacro.DesignForm) {
	...
	nexacro._isCompositeCompElement = function (elem) {
		if (!elem) {
			return false;
		}
	...
	_pDesignForm._hitTestTracker = function (x, y, rootcomp, comp, trackersize) {
		if (comp) {
			...
			if (rect[0] - trackersize <= x && x <= rect[0] + rect[2] + trackersize && rect[1] - trackersize <= y && y <= rect[1] + rect[3] + trackersize) {
				hit = true;
			}
			if (!hit) {
				if (!rootcomp) {
	...
	_pDesignForm.getOverlapComponent = function (compid) {
		...
		for (var i = comp_len - 1; i >= 0; i--) {
			...
			if (_adjust_left <= comp._adjust_left && _adjust_top <= comp._adjust_top && _adjust_rigth >= (comp._adjust_left + comp._adjust_width) && _adjust_bottom >= (comp._adjust_top + comp._adjust_height)) {
	...
	_pDesignForm._showSubLayout = function (obj, bShow, positionstep) {
		...
		if (bShow) {
			...
			obj._adjustPosition = function () {
				...
				var offset_left = (sublayoutmode_info.offset_pos[0] - design_form._scroll_left);
				if (owner_positionstep == -1) {
	...
	_pDesignForm.setThemeUri = function (themename) {
		if (typeof themename == "string") {
			if (themename[0] == "\\" || themename[2] == "\\" || themename[0] == "/" || themename[2] == "/") {
	...
	_pDesignForm._hitTestByRect = function (obj, left, top, right, bottom, type) {
		if (obj) {
			...
			if (type == 0) {
				if (left <= _left && _right <= right && top <= _top && _bottom <= bottom) {
					return true;
				}
			}
			else if (type == 1) {
	...
	_pApplicationAccessPort.InitializeApplicationProperties = function () {
		...
		app.accessibilitytype = "standard";
		app.accessibilitydescreadtype = "label";
		app.accessibilitywholereadtype = "none";
